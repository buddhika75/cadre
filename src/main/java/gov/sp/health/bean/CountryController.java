...
import gov.sp.health.bean.util.JsfUtil;
import gov.sp.health.bean.util.JsfUtil.PersistAction;
...
public class CountryController implements Serializable {
    ...
    @EJB
    private gov.sp.health.facade.CountryFacade ejbFacade;
    ...
    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/BundleCountry").getString("CountryCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/BundleCountry").getString("CountryUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/BundleCountry").getString("CountryDeleted"));
        if (!JsfUtil.isValidationFailed()) {
        ...
        if (selected != null) {
            ...
            try {
                ...
                if (msg.length() > 0) {
                    ...
                    JsfUtil.addErrorMessage(ex, ResourceBundle.getBundle("/BundleCountry").getString("PersistenceErrorOccured"));
                }
            } catch (Exception ex) {
