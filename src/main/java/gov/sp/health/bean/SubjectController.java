...
public class SubjectController  implements Serializable {
    ...
    List<Subject> lstItems;
    private Subject current;
    private DataModel<Subject>  items = null;
    ...
    public List<Subject> getLstItems() {
        return getFacade().findBySQL("Select d From Subject d");
    }

    public void setLstItems(List<Subject> lstItems) {
    ...
    public DataModel<Subject>  getItems() {
        String temSql;
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (Subject) items.getRowData();
    ...
    public Subject searchItem(String itemName, boolean createNewIfNotPresent) {
        Subject searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Subject) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new Subject();
    ...
    public SubjectFacade getEjbFacade() {
        return ejbFacade;
    }

    public void setEjbFacade(SubjectFacade ejbFacade) {
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Subject) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + SubjectController.class.getName());
            }
        }
    }
}
