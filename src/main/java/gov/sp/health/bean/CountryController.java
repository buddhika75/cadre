...
public class CountryController implements Serializable {
    ...
    List<Country> lstItems;
    private Country current;
    private DataModel<Country> items = null;
    ...
    public List<Country> getLstItems() {
        return getFacade().findBySQL("Select d From Country d");
    }

    public void setLstItems(List<Country> lstItems) {
    ...
    public void setCurrent(Country current) {
    ...
    public DataModel<Country> getItems() {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (Country) items.getRowData();
    ...
    public Country searchItem(String itemName, boolean createNewIfNotPresent) {
        Country searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Country) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new Country();
    ...
    public CountryFacade getEjbFacade() {
        return ejbFacade;
    }

    public void setEjbFacade(CountryFacade ejbFacade) {
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Country) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + CountryController.class.getName());
            }
        }
    }
}
